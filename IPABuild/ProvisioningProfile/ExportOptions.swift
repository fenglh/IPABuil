...
//  IPABuild
//
//  Created by fenglh on 2022/11/30.
//

import Foundation

public struct ExportOptions: Encodable {
    ...
    public enum SigningCertificate<Selector: RawRepresentable>: Encodable where Selector.RawValue == String   {
...
extension ExportOptions {
    ...
    func writeToPath(path: URL) throws {
        let data = try generatePList()
...
extension Encodable {
    ...
    func string(using encoder: JSONEncoder = .init()) throws -> String {
        try data(using: encoder).string()
    }
    
    func dictionary(using encoder: JSONEncoder = .init(), options: JSONSerialization.ReadingOptions = []) throws -> [String: String] {
         try JSONSerialization.jsonObject(with: try data(using: encoder), options: options) as? [String: String] ?? [:]
     }
}
...
extension ExportOptions {
    ...
    public struct Manifest: Encodable {
        ...
        var assetPackManifestURL: String?
    }
    ...
    public init(
        ...
        switch export {
        case let .appStore(_manageAppVersionAndBuildNumber, _uploadSymbols):
            compileBitcode = true
            destination = .export
        ...
        case let .nonAppStore(_manifest, _compileBitcode, onDemandResources, _thinning):
            ...
            manifest =  try? _manifest?.dictionary()
